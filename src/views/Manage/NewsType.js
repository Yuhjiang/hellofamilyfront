...
class AdminNewsType extends Component {
  formRef = React.createRef();

  constructor(props) {
    super(props);
    const columns = this.createColumns(["name", "color", "news_count"]);
    this.state = {
      addLoading: false,
      color: "#fff",
      showAddColor: false,
  ...
  getData = (offset, limited) => {
    ...
    getNewsTypeList({offset, limited}).then(resp => {
      this.setState({
        total: resp.count,
        offset: offset,
    ...
    const columns = columnKeys.map(item => {
      if (item === "color") {
        ...
        return {
          ...
          align: "center",
          render: (text, record) => {
            const {news_count} = record;
            return (<Tag
              color={news_count >= 10 ? "red" : "green"}>{news_count}</Tag>)
          }
        }
      } else {
        return {
  ...
  addFinished = values => {
    ...
    createNewsType(values).then(resp => {
      message.success("成功添加资讯分类");
    }).catch(err => {
      console.log(err);
    }).finally(() => {
      this.setState({
  ...
  onEdit = (id, data) => {
    editNewsType(id, data).then(resp => {
  ...
  onDelete = id => {
    ...
    deleteNewsType(id).then(resp => {
      message.success("成功删除资讯分类");
    }).catch(err => {
      console.log(err);
    }).finally(() => {
      this.setState({
